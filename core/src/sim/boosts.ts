...
// Only for the one call site below, inside `updateBoosts`, never at this module's own
 ...
 * then a uniform pick within that rarity's list — the legacy `DISTRIBUTION` order). Two
...
/**
 * A guaranteed drop at (x, y) — the Gold Corsair's boarding-crew loot
 ...
 * own — the caller announces the drop under its own event name for this one (`crew_looted`), so
...
/**
 * GRAVITY_WELL's per-tick pull on enemy fire: every enemy shot (any kind) has its velocity replaced outright to
 ...
 * `hitOctopi`. A pickup is consumed only when `activateBoost` reports it applied (a WAVE_BLAST —
 * direct or chaos-picked — with no crabs on screen leaves the drop falling).
 */
  ...
  for (const d of list) {
    ...
    if (inReach) {
      ...
      if (result.consumed) {
        ...
        // The Abyssal Huntsman's Mirror reads the drop's OWN boost, `d.boost` —
      ...
      // Not consumed (WAVE_BLAST with no crabs): the drop keeps falling, kept below.
    }
    kept.push(d);
  }
 ...
 * table duration (a direct pickup, `chaosTicks === null`) or RANDOM_CHAOS's 600-900 tick roll —
 * which overrides even an instant or permanent boost's own duration, except where the boost
 ...
 * - SHIELD_BARRIER: a no-op (still consumed) while a shield is already up (applies to a
 ...
 *   picked by chaos (a genuinely timed shield) or `-1` (until broken) for a direct pickup.
 ...
 *   a chaos pick that granted nothing must not later remove a stack on expiry either.
...
function activatePicked(s: GameState, picked: BoostType, chaosTicks: number | null): boolean {
  ...
  if (picked === 'SHIELD_BARRIER') {
    if (s.boosts.shield > 0) return true; // re-pickup while active is a no-op, still consumed
 ...
 * plus whether the pickup should be consumed (false only for a WAVE_BLAST, direct
...
/**
 * Applies SPEED_TAMER's linear slowdown to `v`: `max(0.1, 1 - 0.1*stacks)`, computed with
...
/**
 * Applies ICE_FREEZE's ×0.5 slowdown to a per-tick displacement `v`, alongside `tamed`
 ...
 * Crab movement (the march and a wave's arrival descent) passes `false`. Enemy shots are also
 * slowed, per tick — an earlier version only slowed movement:
...
/**
 * The wave-mode score-decay percentage right now: falls 1% every `SCORE_DECAY.every` ticks on
 * `s.scoreDecay`, floored at `SCORE_DECAY.floorPct`. Applied to a crab kill's base points before
 * SCORE_MULTIPLIER's ×2.
 */
...
/**
 * Advances the wave-mode score-decay clock: only while the run has boosts enabled (so a
