...
/**
 * Abyssal Huntsman (kind 10): the old warden of the Hollow Throne, and the last of
 ...
 *
 * **The sight line and the needle** (`attack`, `pushAimLine`, `tick`): `attack` records the
 * muzzle-to-Octopi segment as a *real* group in `s.aims` — `[muzzle.x, muzzle.y, octopi.x, octopi.y,
 * 0, ticksLeft]`, `decoy = 0` — via `pushAimLine`, which raises `boss_aim` for it (for *every* line
 * pushed, the 40-tick opener and each 12-tick re-aim alike, not just the
 ...
 * "burst of three" sets `b.burst = 3`: firing a needle decrements it, and while it is
 * still above 0 a fresh 12-tick real group is drawn at once, to Octopi's position *at that tick* —
 * each one re-aimed — so the rhythm is line 40 → needle → line 12 →
 ...
 * has run its course (below).
 *
 * **One hunt at a time**: phase 4's own `attackDelay` can run as short as 48 ticks
 ...
 *
 * **Decoys** (`pushAimLine`, phase 3+): two mirror copies of the real line, at `leftX`/
 ...
 * for the opener's own decoys (otherwise the two would read identical values
 ...
 *
 * **The honour guard** (`onPhaseStart`, phase 4 only): one `guard5` squad, one veteran of each of
 * reef 10's own five kinds (`HUNTSMAN_GUARD`,
 * tiers 0-4 — a five-kind roster maps tier for tier, reef 10's own row), in the squad band
 * alongside every other boss's own escort (`SQUAD_BAND`, `sim/bosses/tyrant.ts`'s own escort). Gated
 * on the same cap every escort obeys: skipped outright, no retry later, once 4 or
 ...
 * own `raiseWardenLine`); an earlier draft of this file drew `nextInt(2)` for it, but it was fixed
 * instead, since this squad only ever raises once in a fight (there
 * is only one phase 4) and there is no second firing for a direction to vary against.
 *
 * **Mirror** (`onBoostPickup`, `tickThroughTransition`, `onHit`): the one mechanic of
 * this boss that is not driven by his own timers at all — see `ability`'s own doc below for why. Every
 * one of the fifteen `BoostType`s falls into exactly one of three classes, read off
 * `MIRROR_CLASS`:
 *
 ...
 *   rolls is not a pickup of its own, which is exactly why `sim/boosts.ts`'s own call site
 ...
 *
 * **The Mirror is not a timed ability** (`ability`): every other boss's `ability` fires on
 ...
 *
 * **A phase turn drops the aim, never the mirror** (`onTransition`): `s.aims = []`,
 ...
 *    jitter (`nextInt(BOSS.attackJitter)`). `initialAbilityTimer` draws nothing at all.
 ...
 *    draws nothing and `nextAbilityTimer` draws nothing.
 ...
 *    at all; `spawnSquad` itself draws nothing either.
...
/** One of the three classes every mirrorable `BoostType` falls into; `undefined` for the two that mirror nothing at all (COIN_SHOWER, RANDOM_CHAOS). */
const OFFENCE = 0;
const DEFENCE = 1;
const CONTROL = 2;

/** Every `BoostType` mapped to its mirror class. All fifteen are listed; the two left out of every branch (COIN_SHOWER, RANDOM_CHAOS) fall through `mirrorClass` to `undefined`. */
...
/** How many ticks a pickup of `type` mirrors for: its own table duration, or 300 where that is 0 or −1. Never read for a `type` with no mirror class. */
function mirrorTicks(type: BoostType): number {
  const duration = BOOSTS[type].duration;
  return duration <= 0 ? 300 : duration;
}

/** Points a shield the defence mirror grants absorbs before it is spent (40 player shots, at 1 damage each). */
const HUNTSMAN_SHIELD_HP = 40;

/** How much faster a needle flies while the control mirror is up (×1.25, `castNeedle`'s own `mult1000`). */
const HUNTSMAN_CONTROL_MULT = 1250;

/** Needles a burst still owes once it starts: one in phase 1, three from phase 2 on. */
function burstSize(phase: number): number {
  return phase >= 2 ? 3 : 1;
}

/** Ticks a sight line shows before it fires: 40 for the opener, 12 for every burst re-aim. */
const HUNTSMAN_LINE_OPEN = 40;
const HUNTSMAN_LINE_REAIM = 12;

/** The two mirror x's a decoy pair stands at, about the boss's own x — Void Sovereign's own `castClone` formula (`bosses/void.ts`), copied rather than imported (see the file doc). */
 ...
 * records it in `b.aimX`/`b.aimTicks`, raises `boss_aim` for it (every line,
 * not just the opener), and — from phase 3 on — a parallel decoy pair alongside it, each
 ...
 * than have this recompute an identical one off the same untouched `b.x`; a
...
/** Squad crabs alive right now, summed across every squad — the cap every escort obeys. */
...
/** Squad crabs alive at which the honour guard is skipped outright, no retry later. */
export const HUNTSMAN_GUARD_CAP = 4;

/**
 * The honour guard's roster (reef 10's own row): one veteran of each kind, tiers 0-4. A
...
/** The Mirror's own "ability" timer: fixed, far past any real fight's length, so this boss's ability slot never truly fires — see the file doc for why no draw backs it. */
const HUNTSMAN_ABILITY_TIMER = 100_000;

export const HUNTSMAN_HOOKS: BossHooks = {
  attack(s, b) {
    // A line or burst from a previous attack is still live. No draw, no new line.
    if (b.aimTicks > 0) return;
    b.burst = burstSize(b.phase);
    // Computed once and handed to `pushAimLine` below, which would otherwise
  ...
  ability() {
    // Event-driven, not timed: see `onBoostPickup` below and the file doc.
  },
  initialAbilityTimer: () => HUNTSMAN_ABILITY_TIMER,
  nextAbilityTimer: () => HUNTSMAN_ABILITY_TIMER,
  onHit(s, b) {
    // The defence mirror's shield: absorbs one hit per point, the Verdant Templar's own
  ...
  onBoostPickup(s, b, type) {
    ...
    if (cls === undefined) return; // COIN_SHOWER, RANDOM_CHAOS: mirror nothing
  ...
  tickThroughTransition(_s, b) {
    // The mirror timers run through a transition: the player's own boost keeps counting
  ...
  onTransition(s, b) {
    // A line or burst the turn interrupts is simply lost, nothing fires from it. The
  ...
  onPhaseStart(s, b) {
    if (b.phase !== 4) return;
    if (liveSquadCrabs(s) >= HUNTSMAN_GUARD_CAP) return; // the cap every escort obeys
    // Fixed direction, no draw at all — the Verdant Templar's own `line4`
  ...
  tick(s, b) {
    if (b.aimTicks <= 0) return;
    b.aimTicks -= 1;
    // Every group in `s.aims` counts down together; the real one's own segment is
