...
/**
 * Gold Corsair (kind 8): the old raider of Corsair Cove, who buys his crews with
 ...
 * `lootCrewIfWiped` and `types.ts`'s `Squad.alive`: every squad,
 ...
 *   (`b.kind === 8 && b.effectTicks > 0`), wired in alongside the other reefs 6-10
 *   frame fields.
 ...
 *
 * **The crew cap** (at most 2 crews alive at once): the concrete rule is "if 8 or more squad crabs
 * live, the ability re-arms without spawning" — and that is what `ability` below implements
 * literally: it counts every crab with
 ...
 * the upper bound: two *fresh* eight-crab crews can never stand at once, only a crew
 * newly raised alongside whatever is left of an older, already-thinned one. This literal crab-count
 * rule is stricter in shape than a per-squad cap would be, though the two agree on the upper bound
 * that matters here.
 *
 ...
 *    own — then the unconditional `attackDelay` jitter redraw every boss gets.
 ...
 *    even when the crew cap above is going to throw the result away — how many numbers a firing draws
 *    must never depend on how full the field already is, exactly
 ...
 * of its own — "the transition raises nothing
...
/** How many fixed x positions the axe throw is aimed at (a random x of 5). */
...
/** Boarding's timer: 7-15 s, the same range the Frost Castellan's own Crystals uses. */
function abilityTimer(rng: Rng): number {
  return 420 + rng.nextInt(481);
}

/** Ticks of flash before Spikes' reflect window opens (45 — the Verdant Templar's own wind-up length, reused). */
export const CORSAIR_SPIKES_WINDUP = 45;

/** Ticks the reflect window itself lasts once the flash burns out (240). */
export const CORSAIR_SPIKES_TICKS = 240;

/**
 * Squad crabs alive at which Boarding re-arms without spawning a new crew (at most 2 crews alive at
 * once, expressed as this concrete crab count — see this file's own doc above for what that does
 * and does not guarantee).
 */
export const CORSAIR_CREW_CAP = 8;

/**
 * The roster a boarding crew draws from: reef 8's own five kinds, tiers 0..4. Defined locally
 * exactly as the Verdant Templar's own `TEMPLAR_GUARD` is — the level table for reefs 6-10 lives in
 * `levels.ts`, not this file's to reach into before it exists. `SQUAD_TEMPLATES.crew` is
 * `['3333', '1111']` (tiers 3 then 1), so a crew is four heralds (tier 3) over four heavy crabs
 * (tier 1), not "four armored" as an earlier draft of this roster had it.
 */
export const CORSAIR_ROSTER: readonly CrabType[] = ['armored', 'heavy', 'warden', 'herald', 'bubbler'];

/**
 * The crew's origin x for `side` (0 left, 1 right — one `rngBoss.nextInt(2)` picks the
 ...
 * muzzle } from '../boss'`: `sim/boss.ts` -> `bosses/index.ts` ->
...
export const CORSAIR_HOOKS: BossHooks = {
  attack(s, b) {
    // The axe throw: one target drawn of five; a second, from phase 2 on, at the first's
    // own "opposite-ish" index with no extra draw (`(first + 2) mod 5`).
    const m = muzzle(b);
  ...
  ability(s, b) {
    if (b.phase === 3 && b.burst === 1) {
      // Spikes: phase 3's even firings, alternating with Boarding's crews below. No
    ...
    // Boarding: ONE draw for the side, always made — a fixed draw count regardless of
    // the cap below, exactly the pattern the Frost Castellan's own Crystals raise already
  ...
  onHit(s, b, shot) {
    // Spikes' reflect window: while `b.effectTicks` is running, a hit costs the boss
  ...
  tick(s, b) {
    ...
    // Templar's own `effectTicks`-then-`windup` `tick`: a fresh value `windup` sets is
