...
describe('marchCrabs', () => {
  ...
  it('ends the run when a crab reaches the invasion line', () => {
    ...
    s.crabs = [{ x: 2800, y, kind: 0, type: 'normal', hp: 1, slot: -1, shield: 0, shieldTimer: 0, rallies: 0, rallyTimer: 0, squad: 0, revived: 0 }];
    s.waveTotal = 1;
    marchCrabs(s);
...
describe('updateEnemyShots', () => {
  ...
  it('aims straight down at Octopi below the crab, tagged as a crab shot', () => {
    ...
    s.crabs = [{ x: s.octopi.x, y: 2000, kind: 0, type: 'normal', hp: 1, slot: -1, shield: 0, shieldTimer: 0, rallies: 0, rallyTimer: 0, squad: 0, revived: 0 }];
  ...
  it('aims diagonally with integer velocity', () => {
    ...
    s.crabs = [{ x, y, kind: 0, type: 'normal', hp: 1, slot: -1, shield: 0, shieldTimer: 0, rallies: 0, rallyTimer: 0, squad: 0, revived: 0 }];
    expect(firstShot(s)).toMatchObject({ vx: 66, vy: 88 });
  });

  it('fires on about 2% of wave-1 ticks', () => {
    const s = createGame('rate', PRACTICE_RUN);
    s.crabs = [{ x: 2812, y: 2000, kind: 0, type: 'normal', hp: 1, slot: -1, shield: 0, shieldTimer: 0, rallies: 0, rallyTimer: 0, squad: 0, revived: 0 }];
    let fired = 0;
