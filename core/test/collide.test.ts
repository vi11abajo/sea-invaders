...
describe('hitCrabs', () => {
  it('removes the first overlapping crab and the shot, scoring 10 × wave', () => {
    ...
    s.wave = 2;
    s.crabs = [
      { x: 1000, y: 1000, kind: 0, type: 'normal', hp: 1 },
      { x: 1000, y: 1100, kind: 1, type: 'normal', hp: 1 },
    ];
    s.shots = [
    ...
    hitCrabs(s);
    expect(s.crabs).toEqual([{ x: 1000, y: 1100, kind: 1, type: 'normal', hp: 1 }]);
  ...
  it('a piercing shot deals exactly one hit to an armored crab in a pass, then keeps flying', () => {
    ...
    const s = createGame('t', PRACTICE_RUN);
    s.crabs = [{ x: 1000, y: 1000, kind: 0, type: 'armored', hp: 2 }];
  ...
  it('a piercing shot passing through two stacked armored crabs damages each exactly once', () => {
    const s = createGame('t', PRACTICE_RUN);
    s.crabs = [
      { x: 1000, y: 1000, kind: 0, type: 'armored', hp: 2 },
      { x: 1000, y: 400, kind: 0, type: 'armored', hp: 2 }, // further up the shot's path
    ];
...
describe('hitOctopi', () => {
  ...
  it('destroys a crab that touches Octopi and costs a life', () => {
    const s = createGame('t', PRACTICE_RUN);
    s.crabs = [{ x: s.octopi.x + 400, y: s.octopi.y, kind: 0, type: 'normal', hp: 1 }];
    hitOctopi(s);
...
describe('step with collisions', () => {
  it('spawns the next wave when the last crab dies', () => {
    ...
    s.crabs = [{ x: s.octopi.x, y, kind: 0, type: 'normal', hp: 1 }];
    s.waveTotal = 1;
