...
describe('score decay (spec C7)', () => {
  ...
  it("scales a crab kill's points by the current decay percentage before SCORE_MULTIPLIER doubles it", () => {
    ...
    t.crabs = [{ x: 1000, y: 1000, kind: 0, type: 'normal', hp: 1, slot: -1, shield: 0, shieldTimer: 0, rallies: 0, rallyTimer: 0, squad: 0 }];
...
describe('AUTO_TARGET', () => {
  it('recomputes vx/vy every tick towards the nearest crab (legacy 30%-toward/70%-up blend, no clamp)', () => {
    ...
    s.crabs = [{ x: 4000, y: 760, kind: 0, type: 'normal', hp: 1, slot: -1, shield: 0, shieldTimer: 0, rallies: 0, rallyTimer: 0, squad: 0 }];
  ...
  it('prefers the boss over a farther crab', () => {
    const s = createGame('t', PRACTICE_RUN);
    activateBoost(s, 'AUTO_TARGET');
    s.crabs = [{ x: 5000, y: 100, kind: 0, type: 'normal', hp: 1, slot: -1, shield: 0, shieldTimer: 0, rallies: 0, rallyTimer: 0, squad: 0 }];
    spawnBoss(s, 1);
    s.boss!.x = 1100;
    s.boss!.y = 4700;
    s.shots = [{ x: 1000, y: 5000, vx: 0, vy: -SHOT.speed, kind: 'straight', data: 0 }];
  ...
  it('prefers a crab over a farther boss', () => {
    const s = createGame('t', PRACTICE_RUN);
    activateBoost(s, 'AUTO_TARGET');
    s.crabs = [{ x: 1100, y: 4700, kind: 0, type: 'normal', hp: 1, slot: -1, shield: 0, shieldTimer: 0, rallies: 0, rallyTimer: 0, squad: 0 }];
    spawnBoss(s, 1);
    s.boss!.x = 5000;
    s.boss!.y = 100;
    s.shots = [{ x: 1000, y: 5000, vx: 0, vy: -SHOT.speed, kind: 'straight', data: 0 }];
