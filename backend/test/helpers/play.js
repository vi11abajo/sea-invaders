import { DAILY_RUN, PRACTICE_RUN, REPLAY_MODE, ReplayRecorder, createGame, encodeReplay, step } from '@sea-invaders/core';
...
export function playReplay(seed, ticks, mode = REPLAY_MODE.daily) {
  const run = mode === REPLAY_MODE.daily ? DAILY_RUN : PRACTICE_RUN;
  const game = createGame(seed, run);
  const recorder = new ReplayRecorder(seed, mode, 0, run.lives);
